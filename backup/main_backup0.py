...
Travail Pratique #1 - RÃ©solution par lâ€™exploration (description)
...
class Robot:
    ...
    def mostDirtyRoom(self):
        ...
        for y in range(room.num_lines) :
            for x in range(room.num_columns) :
                dirt = self.state_bdi[(x, y)].get("quantity_dirty")
                jewel = self.state_bdi[(x, y)].get("quantity_jewels")
    ...
    def observe_environment_with_my_sensors(self):
        ...
        for y in range(room.num_lines) :
            for x in range(room.num_columns) :
                self.sensors[(y,x)] = {'quantity_jewels': room.matrix[(y,x)].quantity_jewel, 'quantity_dirty': room.matrix[(y,x)].quantity_dirty, }
            #print(self.sensors[(0, 4)].get("quantity_dirty"))
    ...
    def execute_action_plan(self):
        for action, position in self.action_plan:
            ...
            save_json()
        self.history = []
        #x,y=self.action_plan[len(self.action_plan)]
        self.action_plan=[]
        #self.action_plan.append((self.move,y))
        self.action_plan_position = 0
    ...
    def astar(self,start, end):
        ...
        adjacent_pieces = ((0, -1), (0, 1), (-1, 0), (1, 0),)
        ...
        while len(open_list) > 0:
            ...
            # Generate children
            children = []
            for new_position in adjacent_pieces:
                # Get node position
                node_position = (current_node.position[0] + new_position[0], current_node.position[1] + new_position[1])
                if node_position[0]>=0 and node_position[0]<5 and node_position[1]>=0 and node_position[1]<5 :
            ...
            for child in children:
             # Child is on the closed list
                ...
                child.g = current_node.g + 1- knowledge[child.position].get("quantity_dirty") + knowledge[child.position].get("quantity_jewels")
